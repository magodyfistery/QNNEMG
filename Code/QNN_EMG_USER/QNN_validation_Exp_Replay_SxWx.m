function QNN_validation_Exp_Replay_SxWx(USER_ID, row_position, new_rangeDown, experiment_begin, experiment_end, write_excel, windows_sizes, strides)
%%%%CAMBIODANNYINICIO%%%%
if nargin < 6
    write_excel = true;    
end
...
if write_excel
    ...
    for index_windows_sizes=1:numel(windows_sizes)
        ...
        for index_strides=1:numel(strides)
            ...
            add_filename_window_stride = "_windowSize" + window_size + "_stride" + strides(index_strides) + ".xlsx";
            ...
            if index_strides == numel(strides) && index_windows_sizes == numel(windows_sizes)
                row_position = write_excel_wind_stride_validation_base(dir_filename_window_stride, USER_ID, row_position);
            else
                write_excel_wind_stride_validation_base(dir_filename_window_stride, USER_ID, row_position);
            end
...
for experiment_id=experiment_begin:experiment_end
    ...
    for index_window=1:numel(windows_sizes)
        ...
        for index_stride=1:numel(strides)
            ...
            %%%%CAMBIODANNYINICIO%%%%
            s2 = ""+experiment_id+"_windowSize"+window_size+"_stride"+stride;
            ...
            if write_excel
                add_filename_window_stride = "_windowSize" + window_size + "_stride" + stride + ".xlsx";
                dir_filename_window_stride = replace(filename_experimentsQNN, ".xlsx", add_filename_window_stride);   
                if index_stride == numel(strides) && index_window == numel(windows_sizes)
                    row_position = write_experiment_wind_stride_row(dir_filename_window_stride, parameters_validation, row_position, summary, experiment_id, USER_ID);
                else
