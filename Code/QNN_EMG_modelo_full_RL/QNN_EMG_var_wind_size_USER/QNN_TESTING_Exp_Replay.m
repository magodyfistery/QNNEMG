...
assignin('base','WindowsSize',  best_window_size);   %CAMBIAR - 200

%%%%CAMBIODANNYINICIO%%%%
fprintf("User id: %d, Experiment: %d, Window size: %d, stride:%d\n", USER_ID, experiment_id, best_window_size, best_stride);
%%%%CAMBIODANNYFIN%%%%

%%%%PARAMETRIZACION_DANNY%%%%
assignin('base','Stride',  best_stride);        %CAMBIAR - 20
RepTraining        = parameters_testing(experiment_id, index_RepTraining);       %numero de muestras que voy a usar en el entrenamiento x cada usuario en la carpeta C:\Users\juanp\Desktop\QNN - EMG - RandomData - Copy\Data\Specific
...
load(dir_model);     %CAMBIAR -> Aqui se carga el modelo entrenado
  s1 = 'results/csv_full_test_data/Full_dataTESTING_';
  assignin('base','s1', s1);  
  assignin('base','s2', s2); 
  s3 = '.csv';
  assignin('base','s3', s3); 
...
if write_excel
    new_row_position = write_experiment_testing_row(parameters_testing, SHEET_NAME, new_row_position, summary, experiment_id, USER_ID);
