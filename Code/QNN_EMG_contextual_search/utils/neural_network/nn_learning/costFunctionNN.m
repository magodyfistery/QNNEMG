function [J, gradient] = costFunctionNN(neurons_per_layer, thetas_rolled, X, y, lambda)
  ...
  for i=1:number_layers-1
    ...
    index_activations_end += neurons_per_layer(i+1)+1;
  ...
  h = activations_functions(ind(1):ind(2))(2:end)';  % ignora el bias
  delta = h - sparse_y;
  ...
  while i >= 1
    ...
    theta = reshape(thetas_rolled(ind_theta(1):ind_theta(2)), neurons_per_layer(i)+1, neurons_per_layer(i+1))(2:end,:)';
